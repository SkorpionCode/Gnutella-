                ...
                // Decrease the TTL by 1 before forwarding
                    ...
                    for (int i=0; i < messageIDTableSize; i++ ) {
                        if(strcmp(messageIDTable[i].message_id, hit.message_id) ==0) {
                            ...
                            printf("\n %d is nextpeerport, %s is next_peer_ip \n", messageIDTable[i].port, messageIDTable[i].upstream_peer);
                        }
                    }
                    ...
                    if (next_peer_ip && next_peer_port) {
                        ...
                        //sendHitMessage(hit, next_peer_ip, next_peer_port);
                    }
                }
            }
        }
...
void *connect_to_server(void *arg) {
    ...
    strncpy(message.message_id, "1001", MAX_MESSAGE_ID_LENGTH);
    message.ttl = 7;
